...
def tokenize2(data_train,data_test,max_num=100000):
    train = []
    test = []
...
class TextData():
    ...
    def fetch_csv(self,path=None):
        ...
        self.vocab.index_dataset(self.train_set,self.val_set,self.test_set,field_name='text',new_field_name='words')
        
        self.train_set.apply_field(lambda x : len(x),field_name='words',new_field_name='seq_len')
        self.val_set.apply_field(lambda x : len(x),field_name='words',new_field_name='seq_len')
        ...
        self.val_set.apply_field(self.seq_regularize,field_name='words',new_field_name='words')
        ...
        self.val_set.apply(lambda x : int(x['class']),new_field_name="target",is_target=True)
